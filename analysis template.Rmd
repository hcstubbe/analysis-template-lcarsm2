...
for (i in uuidpk) {
  ix = all_data$uidpk == i
  i_usub_id = as.integer(paste0(all_data$participant_pk[ix], all_data$visit_pk[ix], all_data$submission_pk[ix]))
...
all_data_wide <- all_data %>%
    pivot_wider(
        names_from = c(visit_id, submission_n, variable_id, variable_unit), # if you want each submission as row, remove submission_n below
